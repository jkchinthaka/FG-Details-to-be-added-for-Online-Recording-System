...
function fail(itemId: string) {
  return { itemId, value: { kind: "status" as const, value: "FAIL" as const }, remark: "Failed" };
}
...
describe("computeRecommendedLoadingDecision", () => {
  it("recommends APPROVED_FOR_LOADING when every item passes (All Conditions Passed)", () => {
    const items = [makeItem({ id: "a" }), makeItem({ id: "b" }), makeItem({ id: "c", isCriticalFailure: true })];
  ...
  it("recommends LOADING_BLOCKED when any critical item fails, even if other items pass", () => {
    const items = [makeItem({ id: "a" }), makeItem({ id: "insects_presence", isCriticalFailure: true })];
    const responses: ChecklistResponseMap = { a: pass("a"), insects_presence: fail("insects_presence") };
  ...
  it("prioritizes LOADING_BLOCKED over CONDITIONALLY_APPROVED when both a critical and non-critical item fail", () => {
    const items = [makeItem({ id: "door_lock", isCriticalFailure: true }), makeItem({ id: "floor" })];
    const responses: ChecklistResponseMap = { door_lock: fail("door_lock"), floor: fail("floor") };

    expect(computeRecommendedLoadingDecision(items, responses).decision).toBe("LOADING_BLOCKED");
  });
});
...
describe("isOverrideToApprovedAllowed / allowedFinalDecisions", () => {
  it("never allows an approved outcome once the recommendation is LOADING_BLOCKED", () => {
    ...
    expect(allowedFinalDecisions("LOADING_BLOCKED")).toEqual(["LOADING_BLOCKED", "REJECTED"]);
  });
...
describe("createTruckDraftSchema", () => {
  it("accepts a vehicleId-only payload", () => {
    expect(createTruckDraftSchema.safeParse({ vehicleId: "vehicle-1" }).success).toBe(true);
  });

  it("accepts a manual freezerTruckNumber + vehicleNumber payload", () => {
    expect(
      createTruckDraftSchema.safeParse({ freezerTruckNumber: "FT-09", vehicleNumber: "WP AB-1111" }).success,
    ).toBe(true);
  });
  ...
  it("accepts an optional reinspectionOfRecordId", () => {
    expect(
      createTruckDraftSchema.safeParse({ vehicleId: "vehicle-1", reinspectionOfRecordId: "record-1" }).success,
    ).toBe(true);
  });
});
...
describe("loadingDecisionInputSchema", () => {
  it("accepts every final decision value", () => {
    for (const decision of ["APPROVED_FOR_LOADING", "CONDITIONALLY_APPROVED", "LOADING_BLOCKED", "REJECTED"]) {
      expect(loadingDecisionInputSchema.safeParse({ decision }).success).toBe(true);
    }
  });

  it("rejects PENDING as a final decision", () => {
    expect(loadingDecisionInputSchema.safeParse({ decision: "PENDING" }).success).toBe(false);
  });
});
