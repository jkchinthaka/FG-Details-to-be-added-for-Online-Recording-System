...
import { RECORD_STATUSES, RECORD_STATUS_LABELS, RECORD_TYPE_META, RECORD_TYPES, recordTypeForDocumentCode } from "./records";

describe("recordTypeForDocumentCode", () => {
  it("resolves the daily cleaning and freezer truck document codes", () => {
    expect(recordTypeForDocumentCode("NMS/PPU/CL/24")).toBe("DAILY_CLEANING_VERIFICATION");
  ...
  it("round-trips every declared record type's own document code", () => {
    for (const recordType of RECORD_TYPES) {
      expect(recordTypeForDocumentCode(RECORD_TYPE_META[recordType].documentCode)).toBe(recordType);
    }
  });
});
